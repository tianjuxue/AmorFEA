...
def plot_sol(args, case_number):
    ...
    path = args.root_path + '/' + args.solutions_path + '/robot/deploy/u' + str(case_number) + '000000.vtu'
    ...
    tpc = plt.tripcolor(x[:,0], x[:,1], tri, colors, shading='flat', alpha=0.5, vmin=0, vmax=2)
    tpc = plt.tripcolor(xu[:,0], xu[:,1], tri, colors, shading='flat', alpha=1, vmin=0, vmax=2)
    plt.scatter(target_point[0] + 0.25, target_point[1] + 10, marker='*', s=200, color='red')
    ...
    fig.savefig(args.root_path + '/images/robot/sol' + str(case_number) + '.png', bbox_inches='tight')
...
def plot_robot_and_trajectory(args):
    ...
    for i in range(len(x_s)):
        ...
        fig = plt.figure(figsize=(8, 8))  
        colors = np.sqrt(u[:, 0]**2 + u[:, 1]**2 + u[:, 2]**2)
        tpc = plt.tripcolor(xu[:,0], xu[:,1], tri, colors, shading='flat', vmin=0, vmax=2)
...
def plot_walltime(case_number, trun_ad, trun_mix):
    ...
    fig = plt.figure()
    ax = fig.gca()
    ax.plot(wall_time_ad[:trun_ad], objective_ad[:trun_ad], linestyle='--', marker='o', color='blue', label='Adjoint Method') 
    ax.plot(wall_time_mix[nn_number-1:trun_mix], objective_mix[nn_number-1:trun_mix], linestyle='--', marker='o', color='blue')
    ax.plot(wall_time_mix[:nn_number], objective_mix[:nn_number], linestyle='--', marker='o', color='red', label='AmorFEA')
    # ax.set_xscale('log')
    ax.set_yscale('log')
    ax.legend(loc='center right', prop={'size': 12})
    ax.tick_params(labelsize=14)
    fig.savefig(args.root_path + '/images/robot/walltime' + str(case_number) + '.png', bbox_inches='tight')
