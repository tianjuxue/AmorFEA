...
def model_prediction(source, graph, model):
    ...
    source = np.matmul(graph.reset_matrix_boundary, boundary) + \
        np.matmul(graph.reset_matrix_interior, source)
    ...
    for i in range(graph.num_vertices):
        for j in graph.ordered_adjacency_list[i]:
            ...
            line = plt3d.art3d.Line3D(x1, x2, x3,
                                      color=[(x3[0] - min_att) / range_att,
                                             0., 1 - (x3[0] - min_att) / range_att],
                                      linewidth=1)
            ax.add_line(line)
